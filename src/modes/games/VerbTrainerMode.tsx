...
import { getVerbIds, VERB_DATA } from '../../data/verb-deck';
...
export default function VerbTrainerMode({ words, onDone }: Props) {
  const vbIds = getVerbIds();
  const vbWords = useMemo(() => words.filter(w => vbIds.has(w.id)), [words, vbIds]);
  const wordMap = useMemo(() => {
    const m = new Map<string, VocabWord>();
    vbWords.forEach(w => m.set(w.id, w));
    return m;
  }, [vbWords]);

  // Verb metadata lookup
  const verbMeta = useMemo(() => {
    const m = new Map<string, typeof VERB_DATA[0]>();
    VERB_DATA.forEach(v => m.set(v.id, v));
    return m;
  }, []);

  const allVbIds = useMemo(() => vbWords.map(w => w.id), [vbWords]);
  const initialStats = useMemo(() => getSRStats(allVbIds), [allVbIds]);

  const [sessionQueue, setSessionQueue] = useState<string[]>(() => {
    const { newIds, dueIds } = getDueCards(allVbIds);
    const shuffledNew = [...newIds].sort(() => Math.random() - 0.5).slice(0, NEW_CARDS_PER_SESSION);
    const shuffledDue = [...dueIds].sort(() => Math.random() - 0.5);
    return [...shuffledDue, ...shuffledNew].slice(0, CARDS_PER_SESSION);
  });
  ...
  const currentMeta = currentWordId ? verbMeta.get(currentWordId) : undefined;
  ...
  const restart = useCallback(async () => {
    ...
    const fresh = all.filter(w => vbIds.has(w.id));
    const freshIds = fresh.map(w => w.id);
    const { newIds, dueIds } = getDueCards(freshIds);
    const shuffledNew = [...newIds].sort(() => Math.random() - 0.5).slice(0, NEW_CARDS_PER_SESSION);
    const shuffledDue = [...dueIds].sort(() => Math.random() - 0.5);
    const queue = [...shuffledDue, ...shuffledNew].slice(0, CARDS_PER_SESSION);

    // Update word map
    fresh.forEach(w => wordMap.set(w.id, w));
  ...
  if (vbWords.length === 0) {
    return (
      <div className="max-w-lg mx-auto text-center py-12 space-y-4">
        ...
        <h3 className="font-['Oswald'] text-xl font-bold text-soviet-100 uppercase">Keine Verben</h3>
        <p className="text-soviet-300">Verb-Datenbank nicht gefunden.</p>
  ...
  if (totalCards === 0 || sessionQueue.length === 0) {
    const stats = getSRStats(allVbIds);
    return (
      <div className="max-w-lg mx-auto text-center py-12 space-y-4">
        <p className="text-gold-400 text-5xl">&#9733;</p>
        <h3 className="font-['Oswald'] text-xl font-bold text-soviet-100 uppercase">Alle Verben gelernt!</h3>
  ...
  if (isComplete) {
    ...
    return (
      <div className="max-w-lg mx-auto text-center py-12 space-y-4">
        ...
        <h3 className="font-['Oswald'] text-xl font-bold text-soviet-100 uppercase">Verben-Sitzung abgeschlossen!</h3>
        <p className="text-3xl font-bold text-gold-400 font-['Oswald']">{sessionScore.good}/{sessionScore.total}</p>
        <p className="text-soviet-300">Verben gewusst ({accuracy}%)</p>

        <div className="bg-soviet-900/60 rounded-lg p-4 border border-soviet-700 space-y-2">
          <h4 className="font-['Oswald'] text-gold-400 text-sm uppercase tracking-wide">Verben-Fortschritt</h4>
          <div className="flex gap-1 h-4 rounded overflow-hidden bg-soviet-800">
            {stats.learnedCount > 0 && (
              <div className="bg-gold-500 transition-all" style={{ width: `${(stats.learnedCount / allVbIds.length) * 100}%` }} />
            )}
            {stats.dueCount > 0 && (
              <div className="bg-soviet-500 transition-all" style={{ width: `${(stats.dueCount / allVbIds.length) * 100}%` }} />
            )}
          </div>
  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      <div className="flex items-center justify-between mb-4">
        <span className="text-sm text-gold-400 font-['Oswald'] uppercase tracking-wide">Verben &mdash; {dirLabel}</span>
      ...
      <div
        ...
        className={`relative bg-soviet-900/60 rounded-xl border-2 transition-all duration-300 min-h-[300px] flex flex-col items-center justify-center p-8 ${
      ...
      >
        ...
        {currentMeta && (
          <div className="absolute top-4 left-4 right-4 flex gap-2 flex-wrap">
            <span className="text-xs px-2 py-0.5 rounded-full bg-soviet-700 text-soviet-300">{currentMeta.verb}</span>
            <span className="text-xs px-2 py-0.5 rounded-full bg-soviet-700 text-soviet-300">{currentMeta.aspekt}</span>
            <span className="text-xs px-2 py-0.5 rounded-full bg-soviet-700 text-soviet-300">{currentMeta.form}</span>
            {currentMeta.person && (
              <span className="text-xs px-2 py-0.5 rounded-full bg-soviet-700 text-soviet-300">{currentMeta.person}</span>
            )}
          </div>
        )}
        ...
        {!flipped ? (
          <>
            <p className="text-3xl font-bold text-gold-400 text-center leading-relaxed mt-6">{front}</p>
          ...
          <>
            <p className="text-lg text-soviet-400 text-center mb-2 mt-6">{front}</p>
            {direction === 'ru-de' && currentWord.transliteration && (
              <p className="text-xs text-soviet-600 mb-3">({currentWord.transliteration})</p>
            )}
            <div className="w-16 h-0.5 bg-gold-500/30 mb-3" />
            ...
            {/* Example sentence */}
            {currentMeta && currentMeta.example_ru && (
              <div className="mt-4 pt-3 border-t border-soviet-700/50 text-center">
                <p className="text-sm text-soviet-300 italic">{currentMeta.example_ru}</p>
                <p className="text-xs text-soviet-500 mt-1">{currentMeta.example_de}</p>
              </div>
            )}
          </>
        )}
